...
void check_longest(t_ls *ls, t_dir_ptr *current_dir, struct dirent *entry)
{
    ...
    if (itoa_str && ft_strlen(itoa_str) > current_dir->longest_hlsize)
...
static char list_char(mode_t *perms, int perm)
{
    ...
    if (((*perms) & S_IRUSR && perm == S_IRUSR) || ((*perms) & S_IRGRP && perm == S_IRGRP) || ((*perms) & S_IROTH && perm == S_IROTH))
        return ('r');
    if (((*perms) & S_IWUSR && perm == S_IWUSR) || ((*perms) & S_IWGRP && perm == S_IWGRP) || ((*perms) & S_IWOTH && perm == S_IWOTH))
        return ('w');
    if (((*perms) & S_IXUSR && perm == S_IXUSR) || ((*perms) & S_IXGRP && perm == S_IXGRP) || ((*perms) & S_IXOTH && perm == S_IXOTH))
        return ('x');
    return ('-');
}
...
void print_list(struct stat *st, t_dir_ptr *ptr)
{
    ...
    grp = getgrgid(st->st_gid);
    {
        ...
        tmp_time = localtime(&st->st_mtime);
        if (tmp_time)
            now_t = *tmp_time;
        else
            ft_memset(&now_t, 0, sizeof(struct tm));
    }
    ft_printf("%c", is_directory(st));
    ft_printf("%c%c%c", list_char(&perms, S_IRUSR), list_char(&perms, S_IWUSR), list_char(&perms, S_IXUSR));
